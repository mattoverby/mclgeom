...
// Reads VTK tet mesh
bool readVTK(const std::string &vtk, Eigen::MatrixXd &V, Eigen::MatrixXi &T)
{
	...
	if (!file_exists(vtk)) { return ret_err("No vtk file"); }
	...
	{
		std::string line;
		while( std::getline(vtk_file, line) ){ lines.emplace_back(line); }
		vtk_file.close();
	}

	if (lines.size()==0) { return ret_err("No data"); }
	...
	// Get pts and tets
	{
		...
		lV >> buff >> n_pts;
		if (n_pts==0) { return ret_err("No points"); }
