...
template<typename MatrixType>
void
LBFGS<MatrixType>::reset(int rows, int cols)
{
...
template<typename MatrixType>
void
LBFGS<MatrixType>::initialize(MatrixType& x)
{
    initialized = true;
    ...
    if (alpha.rows() != options.M || grad.rows() != x.rows() || grad.cols() != x.cols()) {
        reset(x.rows(), x.cols());
    }
...
template<typename MatrixType>
typename LBFGS<MatrixType>::Scalar
LBFGS<MatrixType>::bracketing_weakwolfe(MatrixType& x, MatrixType& g, const MatrixType& drt, Scalar& step) const
{
    using namespace Eigen;
...
template<typename MatrixType>
bool
LBFGS<MatrixType>::default_converged(Scalar curr_obj,
...
template<typename MatrixType>
typename LBFGS<MatrixType>::Scalar
LBFGS<MatrixType>::inner(const MatrixType& a, const MatrixType& b) const
{
