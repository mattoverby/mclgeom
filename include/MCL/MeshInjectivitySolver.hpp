...
class MeshInjectivitySolver
{
  public:
    struct Options
    {
        int max_iterations = 1000;       ///< max solver iterations
        bool exit_on_injectivity = true; ///< exit if injectivity met (not target volumes)
    } options;

    /// @brief Clears existing data, does not reset options
    void clear();

    /// @brief Adds Dirichlet boundary conditions on the vertices
    void add_pins(const int* pinned_vertex_inds, int num_pinned_vertices);

    /// @brief Moves vertices to best satisfy all constraints. x_rest may be null if unavailable,
    /// but a good "rest" configuration is helpful for scaling the gradients.
