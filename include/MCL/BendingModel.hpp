...
/// @brief Computes cotangent between (v1-v0) and (v2-v0)
template<typename T>
inline T cotangent(
...
/// (x0, x1) is the shared edge, with tris (x0, x1, x2) and (x1, x0, x3).
template<typename T>
inline Eigen::RowVector4<T> quadratic_K0(
    const Eigen::Vector3<T>& x0,
...
/// (x0, x1) is the shared edge, with tris (x0, x1, x2) and (x1, x0, x3).
template<typename T>
inline Eigen::Matrix4<T> quadratic_Q(
    const Eigen::Vector3<T>& x0,
    const Eigen::Vector3<T>& x1,
    const Eigen::Vector3<T>& x2,
    const Eigen::Vector3<T>& x3)
{
    Eigen::RowVector4<T> K = quadratic_K0(x0, x1, x2, x3);
    T A1 = 0.5 * ((x1 - x0).cross(x2 - x0)).norm();
    T A2 = 0.5 * ((x0 - x1).cross(x3 - x1)).norm();
    T scale = std::abs(A1 + A2) > 0 ? T(3 * (A1 + A2)) : T(1);
    Eigen::Matrix4<T> Q = scale * (K.transpose() * K);
    return Q;
}
...
inline void make_hinges(const DerivedF &F, DerivedH &H, bool include_boundary=false)
{
    struct Entry
    {
        int x0, x1;
