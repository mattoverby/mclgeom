...
class Logger
{
  ...
  public:
    ...
    void increment_frame()
    {
        std::lock_guard<std::mutex> guard(write_mutex);
        curr_frame++;
        while ((int)frame_data.size() < curr_frame + 1) {
            frame_data.emplace_back();
        }
        if (curr_frame >= 0) {
            frame_data[curr_frame].start_timer("elapsed_ms");
        }
    }
    ...
    void stop_frame()
    {
        ...
        if (curr_frame >= 0) {
            frame_data[curr_frame].stop_all_timers();
        }
    }
    ...
    void add_runtime_s(double s, int frame = -2)
    {
        ...
        if (curr_frame == -1) {
            init_data.add_time("elapsed_ms", s);
        } else if (curr_frame >= 0) {
            while ((int)frame_data.size() < frame + 1) {
                frame_data.emplace_back();
            }
    ...
    void get_timings(std::unordered_map<std::string, std::vector<double>>& timings_ms)
    {
        ...
        for (int i = 0; i < num_data; ++i) {
            for (LabelIt it = labels.begin(); it != labels.end(); ++it) {
                ...
                while ((int)per_frame_ms.size() < i + 1) {
                    per_frame_ms.emplace_back(0);
                }
                // Set elapsed if present in the frame data
                DoubleIt it_ms = frame_data[i].timings_ms.find(it->first);
                if (it_ms != frame_data[i].timings_ms.end()) {
                    per_frame_ms[i] = it_ms->second;
                }
            }
        }
    }
    ...
    void write_csv(const std::string& csv_name = "mcl_log.csv")
    {
        ...
        for (int curr_write_frame = -1; curr_write_frame < num_frames; ++curr_write_frame) {
            ...
            for (it = timings_ms.begin(); it != timings_ms.end(); ++it) {
                double ms = (curr_write_frame == -1) ? init_data.timings_ms[it->first]
