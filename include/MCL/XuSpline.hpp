...
public:
	...
	T ddg(T x) const { (void)(x); return 0.5*lambda; }
	T ddh(T x) const { (void)(x); return 0.0; }
};
...
public:
	...
	T dg(T x) const { (void)(x); return lambda; }
	T dh(T x) const { return XuSpline<T>::d_compress_term(kappa,x); }
	T ddf(T x) const { (void)(x); return 2.0*mu + lambda; }
	T ddg(T x) const { (void)(x); return 0.0; }
	T ddh(T x) const { (void)(x); return 0.0; }
};
